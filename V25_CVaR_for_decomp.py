...
show_plots = False # Used to toggle between plotting and not plotting...
...
eta = model.addVars(W, lb=0, vtype=GRB.CONTINUOUS, name="eta")
...
# CVaR constraints
model.addConstrs((eta[w] >= zeta-
                        gp.quicksum((p_DA[t]*lambda_DA[w,t] + 
                        p_RES[t]*lambda_RES[w,t] - # New
                        (Delta_down[w,t]+a_RES[w,t]*gamma_RES[w,t])*lambda_B[w,t]) # Changed
                        for t in TT)  for w in WW), name="c_CVaR")
