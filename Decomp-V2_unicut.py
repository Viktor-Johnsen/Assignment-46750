...
def solve_mas(W, T, pi, p_RT, lambda_B, lambda_DA, lambda_RES, gamma_RES, P_nom):
    ...
    LB_sum_arr = np.zeros(VMAX)
    UB_sum_arr = np.zeros(VMAX)
    ...
    for v in range(VMAX):
        ...
        if model.status == GRB.OPTIMAL:
            ...
            timelist.append(time.time()-tstart)
            #print(f"UB: {UB}, LB: {LB}")
    ...
    if model.status == GRB.OPTIMAL:
         return model.objVal, p_DA_val[:,v], p_RES_val[:,v], v, LB_sum_arr, UB_sum_arr, timelist
...
obj, p_DAs, p_Res, v, LB_sum_arr, UB_sum_arr, timelist = solve_mas(W, T, pi, p_RT, lambda_B, lambda_DA, lambda_RES, gamma_RES, P_nom)
end_time = time.time()
...
plt.plot(timelist, LB_sum_arr[:v], label='Lower Bound', marker='o', markersize=4)
plt.plot(timelist, UB_sum_arr[:v], label='Upper Bound', marker='o', markersize=4)
plt.xlabel('Time (seconds)', fontsize=16)  # Increased font size
plt.ylabel('Bound Value', fontsize=16)     # Increased font size
plt.ylim(0, 40000)  # Set y-axis limit
plt.title('Convergence of Lower and Upper Bounds, uni-cut', fontsize=16)
plt.legend(fontsize=14)  # Increased legend font size
plt.grid(True)
plt.xticks(fontsize=14)  # Increased font size for x-axis values
plt.yticks(fontsize=14)  # Increased font size for y-axis values
plt.show()